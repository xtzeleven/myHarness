---
name: cross-stage-check
description: 产研全链路文档间一致性检查（文档↔文档漂移，非代码↔文档）。**触发场景**：「查一下文档之间有没有漂移」「roadmap 和 ADR / CHANGELOG 状态对得上吗」「M8 到底启动了没，各文档口径一致吗」「新增 ADR / roadmap 大改后扫一遍一致性」「MEMORY.md 索引指的文件还在吗」。扫 roadmap / ADR / CHANGELOG / MEMORY.md / AGENTS.md / m8-*.md / improvement-backlog 之间的不一致。**不适用**：代码↔文档漂移（用 sync-docs / docs-keeper agent）。
---

# Cross-Stage Consistency Check

**跨阶段一致性检查**：扫产研全链路**文档之间**的漂移。元能力，不动文件，**只产清单**。承担 [ADR-0008](../../../docs/adr/0008-process-capability-expansion.md) 能力 H。
...
- **不调 agent**：本能力是元能力，避免与 docs-keeper / 其他 agent 重叠
...
- 每次会话末，作为收尾自检（与 `audit-practices` 互补：本能力查文档间一致性，audit-practices 查工程化机制）
