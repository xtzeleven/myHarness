---
name: audit-practices
description: 工程实践自检 — 对照 .claude/rules/engineering-practices.md 15 维度逐项打 ✅/⚠️/❌ 并产报告。**触发场景**：用户说"自检下工程实践 / 工程化做得怎么样 / 有没有不达标的地方 / audit 一下 / 检查 Harness 完备性 / 这项目工程质量如何"。也可由 /audit-practices 命令显式触发。判据真源在本文，命令是薄入口。
---

# audit-practices — 工程实践自检
...
## 参数（由 /audit-practices 命令透传）

- 无参数：跑全量 15 维度
- 指定关键词（如 `ddd` / `mcp` / `hooks` / `ci` / `policy`）：仅对该维度做深度审查并给修复样例
