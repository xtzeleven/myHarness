...
| G15 | 决策 `.claude/` 与 `plugin/` 共存期长度：是否在 M8' 完成后清空原 `.claude/`，让仓库根成为纯 plugin 仓库 | —      | ⏸️ ADR-0006 已产出（推荐选项 B），待用户拍板                                     |
...
**最近更新**：2026-05-12
**进度快照**：G1-G14 全部 ✅；G15 决策文档（ADR-0006）已产出，待用户拍板。本会话另外修复了 H1-H5 / M1/M2/M4/M5 / S2-S5 / F1-F4 共 16 项跨"装上就坏"/"功能性退化"/"结构性缺陷"的问题。
