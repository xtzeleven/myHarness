...
| D8  | Hook 规则补强：`ask-user.yaml` 的 `pom-major-deps` / `ddd-aggregate-boundary` 等仅 match Edit/Write/MultiEdit，**Bash heredoc / `>` 重定向写敏感文件不被拦**。2026-05-21 M8-T1 写 pom.xml 实测命中盲点（已手动 audit log 留痕 + 用户 Q1 显式授权）。修法：加 Bash + `cmd_matches: '(cat | tee                                                                                            | echo)\s\*[^ | ]*>\s*pom\.xml'` 等规则覆盖。 | M8-T3+ 真要改 pom.xml / domain 时这个盲点会被频繁踩。建议 M8-T1 commit 后立即修。 |
