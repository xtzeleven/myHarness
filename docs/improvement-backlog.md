...
| #   | 维度            | 项                                                                                                                                          | 工作量 | 关联                                           |
| --- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------- |
| B1  | Hooks           | `UserPromptSubmit` hook：拦截敏感词（`DROP DATABASE` / `rm -rf /` 字面量在 prompt 中）                                                      | 30 min | 提早一步阻止误操作                             |
| B2  | Permission Gate | PostToolUse 加**事后秘钥检测**（grep `password=` / `AKIA*` / `-----BEGIN.*PRIVATE KEY-----`），命中 stderr 红色警告                         | 1 h    | 当前 PostToolUse 只 audit + format，不审查内容 |
| B3  | Tool Router     | 写 `docs/mcp-onboarding.md` — 新增 MCP 的模板（.mcp.json / .env.example / lint.yml 校验 / 文档登记四步清单）                                | 1 h    | M8 加新 MCP 时不必临时摸索                     |
| B4  | Context Manager | `audit-context-cost.py` 加每会话 token baseline 入 audit log（每周 GH Actions 跑）                                                          | 1 h    | 长期追踪 context 膨胀                          |
| B5  | Subagent        | 给 `ddd-architect` / `spring-boot-reviewer` / `schema-analyst` 的 agent 文件末尾加 schema 输出范例（参考 `docs/agent-output-schema.md` §3） | 30 min | 启用 SubagentStop 后才有数据                   |
...
| #   | 项                                                                                                  | 工作量 | 修法                                                         |
| --- | --------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------ |
| C17 | engineering-practices §0 阅读顺序未列 §15                                                           | 2 min  | Edit 加 "元规则：第 15 节"                                   |
| C18 | `.audit.log` 在 .gitignore 靠 `*.log` 通配兜底，与 `.session*` 显式风格不一致                       | 1 min  | 加 `.claude/.audit.log` 显式行                               |
...
| C21 | `memory-conventions.md:62-63` `pref_*` / `session_*` 标 "M4.5 / M5 启用"，实际 MEMORY.md 无对应条目 | 10 min | 决定推进 or 改文档为"未推进"；不要悬挂                       |
