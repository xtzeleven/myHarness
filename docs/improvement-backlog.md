...
| EH  | H1-H7 手工验证全 ✅（详见 [g13-findings.md §H1-H7](g13-findings.md#h1-h7-手工验证结果2026-05-13)）；过程发现 F8 已修 | 2026-05-13 |
| EF8 | F8: Windows 反斜杠路径让 PreToolUse 灰名单全失效（致命跨平台 bug）→ `pre-tool-use.sh` 路径规范化 + 5 case 回归 | ce6ff49 |
...
**最近更新**：2026-05-13（H1-H7 全 ✅ + F8 修复）
**进度快照**：G1-G15 全部 ✅；H1-H7 手工验证 7/7 ✅（过程发现 **F8** Windows 反斜杠路径让 PreToolUse 灰名单全失效，已修 commit `ce6ff49` + 5 case 回归）。本会话另外修复了 H1-H5 / M1/M2/M4/M5 / S2-S5 / F1-F4 / **F8** 共 17 项跨"装上就坏"/"功能性退化"/"结构性缺陷"的问题。
**总耗**：~13 h（plugin 骨架 → 关键挑战 1 → CI 加固 → G13 验证 → G14 措辞 → G15 决策 → H1-H7 手工 + F8）。M8' 完成。
**剩余**：仅 P1 中 B4/B6 两项（详见 §B），与 H1-H7 互不阻塞，可后续单独清。
