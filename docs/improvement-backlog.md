...
| #   | 项                                                                                                      | 工作量 | 关联 / 备注                 |
| --- | ------------------------------------------------------------------------------------------------------- | ------ | --------------------------- |
| G1  | 起 `plugin/` 目录骨架 + `plugin/.claude-plugin/plugin.json`（name / version / description / author）    | 30 min | M8'-T2 起步                 |
| G2  | 复制 `.claude/agents/*` → `plugin/agents/*`（8 个）                                                     | 10 min | 直接 cp，frontmatter 不动   |
| G3  | 复制 `.claude/commands/*` → `plugin/commands/*`（5 个）                                                 | 10 min | 调用名自动变 `/harness:xxx` |
| G4  | 复制 `.claude/scripts/*` → `plugin/scripts/`（或挂 `plugin/bin/`）                                      | 10 min | 决策位置：scripts/ 或 bin/  |
| G5  | 写 `plugin/hooks/hooks.json`（迁 `.claude/settings.json` hooks 段，格式与 settings.json 兼容）          | 30 min | M8'-T3                      |
| G6  | hook 内部路径替换为 `${CLAUDE_PLUGIN_ROOT}/...` + smoke test 全过                                       | 1-2 h  | M8'-T4 关键挑战 2           |
| G7  | 决策审计日志位置：plugin 私有 vs 用户项目根 vs 用户 `~/.harness/`                                       | 30 min | M8'-T6（需用户拍板）        |
...
| G10 | 复制 `engineering-practices.md` → `plugin/rules/`；让 `/harness:audit-practices` 显式 Read              | 15 min | rules 注入路径变更          |
| G11 | 复制 `.mcp.json` → `plugin/.mcp.json`；新增 `plugin/.env.example` 说明 .env 在用户侧维护                | 20 min | MCP 凭据不入 plugin         |
| G12 | 写 `plugin/README.md`（安装 / 用法 / 调用前缀 / 已知限制 / 与 myHarness 仓库的关系）                    | 1 h    | 用户文档                    |
