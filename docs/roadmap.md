...
## 7. M8 — Java DDD 实例化（六维度回归测试）

### 目标
真起 `pom.xml` + `src/`，让 M4-M7 建立的六维度框架在真实代码上验证。

### 子任务（高层，待 M7 完成后细化）
- 起最小 Spring Boot 3 + 一个示例 BC（如 `order` 限界上下文）
- 让 ddd-architect / spring-boot-reviewer / maven-build-doctor / schema-analyst / migration-author 五个后端 agent 各跑一遍真实任务
- M6 子目录 CLAUDE.md 落到 domain/application 等
- M5 周期任务挂上：每 PR 跑 docs-keeper

### 成功标准（六维度回归）
- [ ] **Loop**：长任务（如"加一张表 + ER + 迁移 + 文档"）能跨 sub-agent 串起来，中断可续
- [ ] **Context**：主对话注入 token 数稳定且可观测
- [ ] **Tools**：MCP 真连库跑一次 EXPLAIN，gitnexus 索引真代码
- [ ] **Permission Gate**：尝试改 domain/Aggregate 时被拦下，授权后才进
- [ ] **Memory**：踩过的坑不再踩第二次（如 jq 问题在 M8 不会重现）
- [ ] **Policy**：模型选择按 §15 走，可审计
